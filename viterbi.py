...
lines=['null\t<s>']  #adding line boundaries   
...
tags  = [l.split("\t")[1] for l in lines]  #list of tags in the same sequence as that of wsj corpus               
tl=[i for i in set(tags) if i not in ('</s>','<s>')]  #list of the set tags without word boundaries
tl.insert(0, '<s>')
...
for i, t in enumerate(tags): #storing bigram of tags in a list
    if i+1<len(tags):
        next=tags[i+1]
        bigrams.append(t+next)

for t in set(tags):  # storing emision and transition probabilities
...
def viterbi(s=[]):
    ...
    for j in range (1,len(tl)):  #transitioning from <s> to the best probable state for the first word         
        trelis[j,1]=transition['<s>'][tl[j]]*emision[tl[j]].get(s[0],0)
        pointer[j,1]=0          
            
    for i in range (2,len(s)+1):  #transitions in between the start state and end state
    ...
    for k in range(1,len(tl)):  #transitioning from the best probable state for the last word to </s>
        im[k]=trelis[k,len(s)]*transition[tl[k]]['</s>']    
    trelis[46,len(s)+1]=max(im)
    pointer[46,len(s)+1]=np.argmax(im)
    
    trace=[tl[int(pointer[46,len(s)+1])]]  #tracing back the backpointer trelis from </s> to <s> moving backwards
